...
import { AnyPatchStep, BasePatchStep, DiffSettings, Index, PatchStep, unsafeAssert } from './types';
...
function diffHeuristic(a: unknown, b: unknown, settings: DiffSettings): number {
  ...
  if (Array.isArray(a) && Array.isArray(b)) {
    ...
    let total: string[] = [];
    for (let k in a) total.push(k);
    for (let k in b) if (!(k in a)) total.push(k);
    let change = 0;
    for (let i = 0; i < total.length; i++) {
      if (total[i] in a && !(total[i] in b)) {
        ...
        change +=
          diffHeuristic(
            (a as Record<string, unknown>)[total[i]],
            (b as Record<string, unknown>)[total[i]],
            settings,
          ) * settings.diffMulSameKey;
      }
    }
    if (total.length != 0) return change / total.length;
    return 0;
  } else {
    return a == b ? 0 : 1;
  }
...
function diffArrayHeuristic(a: unknown[], b: unknown[], settings: DiffSettings) {
  ...
  for (let i = 0; i < b.length; i++) {
    ...
    if (validDif > settings.arrayTrulyDifferentThreshold) validSrc = null;
    if (validSrc != null) {
      while (ia < validSrc) {
        sublog.push('POPA');
        ia++;
...
export function diff(a: unknown, b: unknown, settings: Partial<DiffSettings>) {
  ...
  if (settings !== void 0) photomerge(trueSettings, settings);
  if (trueSettings.comment !== void 0) trueSettings.commentValue = trueSettings.comment;
  ...
  if (trueSettings.optimize) {
    for (let i = 1; i < result.length; i++) {
      ...
      if (here['type'] == 'EXIT') {
        if (prev['type'] == 'EXIT') {
          // Crush EXITs
          if (!('count' in here)) here['count'] = 1;
          if (!('count' in prev)) prev['count'] = 1;
          ...
          if ('comment' in here) prev['comment'] = here['comment'];
          optimizedOut = true;
        }
      } else if (here['type'] == 'ENTER') {
        if (prev['type'] == 'ENTER') {
          // Crush ENTERs
          if (!Array.isArray(prev['index'])) prev['index'] = [prev['index']];
          if (!Array.isArray(here['index'])) here['index'] = [here['index']];
...
export function diffApplyComment<T extends BasePatchStep>(step: T, settings: DiffSettings) {
  if (settings.commentValue !== void 0) step.comment = settings.commentValue;
  return step;
}
  ...
  if (Array.isArray(a) && Array.isArray(b)) {
    ...
    for (let i = 0; i < array.length; i++) {
      if (array[i] == 'POPA') {
        log.push(
          diffApplyComment(
            { type: 'REMOVE_ARRAY_ELEMENT', index: bi } as PatchStep.REMOVE_ARRAY_ELEMENT,
            settings,
          ),
        );
        ai++;
      } else if (array[i] == 'INSERT') {
        let insertion = diffApplyComment(
          {
            type: 'ADD_ARRAY_ELEMENT',
            index: bi,
            content: photocopy(b[bi]),
          } as PatchStep.ADD_ARRAY_ELEMENT,
          settings,
        );
        // Is this a set of elements being inserted at the end?
        let j;
        for (j = i + 1; j < array.length; j++)
          if (array[j] != 'INSERT' && array[j] != 'POPA') break;
        // If it is a set of elements being inserted at the end, they are appended
        if (j == array.length) delete insertion['index'];
        log.push(insertion);
        bi++;
      } else if (array[i] == 'PATCH') {
        ...
        if (xd != null) {
          ...
          log.push(
            diffApplyComment(
              { type: 'SET_KEY', index: bi, content: photocopy(b[bi]) } as PatchStep.SET_KEY,
              settings,
            ),
          );
        }
    ...
    for (let k in a) {
      if (k in b) {
        ...
        if (diffHeuristic(a[k], b[k], settings) >= settings.trulyDifferentThreshold) {
          log.push(
            diffApplyComment(
              { type: 'SET_KEY', index: k, content: photocopy(b[k]) } as PatchStep.SET_KEY,
              settings,
            ),
          );
        } else {
          ...
          diffApplyComment(
            {
              type: 'SET_KEY',
              index: k,
              content: photocopy((b as Record<string, unknown>)[k]),
            } as PatchStep.SET_KEY,
            settings,
          ),
        );
  } else if (a != b) {
